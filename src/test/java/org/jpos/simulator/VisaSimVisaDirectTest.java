...
public class VisaSimVisaDirectTest {
    ...
    public static List<TestCase> data() throws Exception {
        ...
        helper = new JposTestRunnerHelper("visa-client-sim-vd");
        return helper.buildData().stream().filter(testCase -> {
                    if (TESTS_TO_FILTER.isEmpty()) {
                        return true;
                    }
                    final String testName = testCase.getFilename().replace("cfg/visa_direct/", "");
