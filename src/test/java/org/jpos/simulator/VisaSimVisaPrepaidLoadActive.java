...
public class VisaSimVisaPrepaidLoadActive {
    ...
    public static List<TestCase> data() throws Exception {
        ...
        helper = new JposTestRunnerHelper("visa-client-sim-prepaid-load-active");
        return helper.buildData().stream().filter(testCase -> {
                    if (TESTS_TO_FILTER.isEmpty()) {
                        return true;
                    }
                    final String testName = testCase.getFilename().replace("cfg/prepaid_load_active/", "");
